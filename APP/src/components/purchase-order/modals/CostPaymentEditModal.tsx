...
import { NumberInput, DateInput, Select } from '../common';
import { colors, spacing } from '../../../constants';
import type { LaborCostItem } from '../tabs/CostPaymentTab';

interface CostPaymentEditModalProps {
  visible: boolean;
  onClose: () => void;
  onSave: (data: CostPaymentEditData) => Promise<void>;
...
export interface CostPaymentEditData {
  unitPrice: number;
  backMargin: number;
  commissionType: string;
  commissionRate: number;
...
export function CostPaymentEditModal({
  visible,
  onClose,
  onSave,
  initialData,
  isSuperAdmin = false,
}: CostPaymentEditModalProps) {
  const [formData, setFormData] = useState<CostPaymentEditData>({
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* 운송비 */}
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>🚚 운송비</Text>
            <NumberInput
              label="업체 배송비"
