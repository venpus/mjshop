...
import { productCollabImageUpload } from '../utils/upload.js';
...
router.post(
  '/products/:id/upload',
  (req, res, next) => {
    productCollabImageUpload.array('images', 20)(req, res, (err: unknown) => {
      if (err) {
        const code = (err as { code?: string }).code;
        const message = (err as Error).message;
        if (code === 'LIMIT_FILE_SIZE') {
          return res.status(413).json({ success: false, error: '파일 크기는 10MB 이하여야 합니다.' });
        }
        ...
        return res.status(400).json({ success: false, error: message || '이미지 업로드 처리 중 오류가 발생했습니다.' });
      }
      next();
    });
  },
