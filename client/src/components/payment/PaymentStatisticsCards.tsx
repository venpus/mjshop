...
export function PaymentStatisticsCards({ refreshTrigger }: PaymentStatisticsCardsProps) {
  ...
  const loadStatistics = async () => {
    ...
    try {
      ...
      // 패킹리스트의 shipping_cost_difference는 지급예정에 포함
      allHistory
        .filter((item) => item.source_type === 'packing_list')
        .forEach((item) => {
          const shippingDiff = item.shipping_cost_difference || 0;
  ...
  const renderDetail = (total: number, detail: PaymentDetail, highlightColor: string) => (
    <div className="flex items-center gap-4">
      {/* 총계 부분 */}
      <div className="flex items-baseline gap-2 px-3 py-2 rounded-lg bg-gray-50 border-2 border-gray-200">
        <span className="text-xs text-gray-500">총계</span>
        <span className={`text-2xl font-bold ${highlightColor}`}>{formatCurrency(total)}</span>
      </div>
      ...
      <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 border border-gray-200">
        <div className="flex items-baseline gap-2 px-2.5 py-1.5 rounded-md bg-blue-50 border border-blue-200">
          <span className="text-xs text-blue-600 font-medium">선금</span>
          <span className="text-lg font-semibold text-blue-700">{formatCurrency(detail.advance)}</span>
        </div>
        <div className="flex items-baseline gap-2 px-2.5 py-1.5 rounded-md bg-orange-50 border border-orange-200">
          <span className="text-xs text-orange-600 font-medium">잔금</span>
          <span className="text-lg font-semibold text-orange-700">{formatCurrency(detail.balance)}</span>
        </div>
        <div className="flex items-baseline gap-2 px-2.5 py-1.5 rounded-md bg-indigo-50 border border-indigo-200">
          <span className="text-xs text-indigo-600 font-medium">배송비</span>
          <span className="text-lg font-semibold text-indigo-700">{formatCurrency(detail.shipping)}</span>
        </div>
      </div>
    </div>
  );
  ...
  const renderAdminCostDetail = (total: number, detail: AdminCostDetail, highlightColor: string) => (
    <div className="flex items-center gap-4">
      ...
      {/* 상세 내역 그룹 */}
      <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 border border-gray-200">
        <div className="flex items-baseline gap-2 px-2.5 py-1.5 rounded-md bg-teal-50 border border-teal-200">
          <span className="text-xs text-teal-600 font-medium">발주 주가비</span>
          <span className="text-lg font-semibold text-teal-700">{formatCurrency(detail.backMargin)}</span>
        </div>
        <div className="flex items-baseline gap-2 px-2.5 py-1.5 rounded-md bg-cyan-50 border border-cyan-200">
          <span className="text-xs text-cyan-600 font-medium">배송비 추가비</span>
          <span className="text-lg font-semibold text-cyan-700">{formatCurrency(detail.shippingDifference)}</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-4 mb-6">
      ...
      <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
        <p className="text-lg font-bold text-gray-900 mb-3">지급된 금액 총계</p>
        {isLoading ? (
      ...
      {/* 지급 예정 총계 */}
      <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
        <p className="text-lg font-bold text-gray-900 mb-3">지급 예정 총계</p>
        {isLoading ? (
          <p className="text-2xl font-bold text-gray-900">-</p>
        ) : (
      ...
      {/* 금일까지 지급요청 총계 */}
      <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
        <p className="text-lg font-bold text-gray-900 mb-3">금일까지 지급요청 총계</p>
        {isLoading ? (
          <p className="text-2xl font-bold text-gray-900">-</p>
        ) : (
      ...
      {/* A레벨 관리자 추가 비용 관련 카드들 (한 줄) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* A레벨 관리자 추가 비용 총계 */}
        <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
          <p className="text-lg font-bold text-gray-900 mb-3">A레벨 관리자 추가 비용 총계</p>
          {isLoading ? (
            <p className="text-2xl font-bold text-gray-900">-</p>
          ) : (
        ...
        {/* A레벨 관리자 지급예정 비용 */}
        <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
          <p className="text-lg font-bold text-gray-900 mb-3">A레벨 관리자 지급예정 비용</p>
          {isLoading ? (
            <p className="text-2xl font-bold text-gray-900">-</p>
          ) : (
        ...
        {/* A레벨 관리자 지급완료 비용 */}
        <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
          <p className="text-lg font-bold text-gray-900 mb-3">A레벨 관리자 지급완료 비용</p>
          {isLoading ? (
            <p className="text-2xl font-bold text-gray-900">-</p>
          ) : (
